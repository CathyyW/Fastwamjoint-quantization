...
# Static per-call clipping and per-stream gamma; optional official rotation.


def _common(x, packed, scales, bias, inverse):
    if x.dtype not in (torch.bfloat16, torch.float16) or not x.is_cuda or x.ndim < 2:
        raise ValueError("W4A4 expects CUDA BF16/FP16 inputs with token and channel axes.")
    if packed.ndim != 2:
        raise ValueError("Packed WAM weights must be a two-dimensional tensor.")
    k, n = x.shape[-1], packed.shape[0]
    if packed.dtype != torch.uint8 or packed.shape != (n, k // 2) or k % 256 or n % 8:
        raise ValueError(f"Unsupported FastWAM W4A4 shape: K={k}, N={n}.")
...
def symmetric_scheduled_stream_linear(x, packed, scales, bias, inverse, activation_scales, gains, token_counts, call):
    ...
    if gains.ndim != 2 or not token_counts or any(c <= 0 for c in token_counts) or len(token_counts) != gains.shape[-1] or sum(token_counts) != x.shape[-2]:
...
def _equal_stream_span(x, gains, token_counts):
    ...
    if len(set(token_counts)) != 1:
        raise ValueError("The Cosmos AdaLN/gate W4A4 producer requires equal-length FastWAM streams.")
    return int(token_counts[0])


def symmetric_scheduled_stream_rht_linear(
    x, packed, scales, bias, inverse, activation_scales, gains, token_counts, call, signs,
):
    """One official rotation/D/gamma/A4 producer followed by INT4 GEMM."""
    x = _common(x, packed, scales, bias, inverse)
    if (gains.ndim != 2 or not token_counts or any(c <= 0 for c in token_counts)
            or len(token_counts) != gains.shape[-1] or sum(token_counts) != x.shape[-2]):
        raise ValueError("WAM rotated token layout does not cover the input.")
    if signs.shape != (x.shape[-1],) or signs.device != x.device or signs.dtype != torch.float32:
        raise ValueError("Rotation signs must be CUDA float32 [K].")
    if len(set(token_counts)) == 1:
        span = int(token_counts[0])
    elif len(token_counts) == 2:
        span = -int(token_counts[1])
    else:
        raise ValueError("Unsupported rotated WAM stream layout.")
    return load_extension().symmetric_static_stream_scheduled_rht(
        x, signs.contiguous(), packed, scales, bias, inverse,
        activation_scales.contiguous(), gains.contiguous(), span, int(call), True,
    )
...
def symmetric_scheduled_stream_adaln_linear(
    ...
    """Move affine-free LayerNorm and FastWAM's MLP AdaLN into A4 producer."""
    x = _common(x, packed, scales, bias, inverse)
    ...
    if (adaln_scale.shape != adaln_shift.shape or adaln_scale.shape[-1] != x.shape[-1]
            ...
            or adaln_shift.dtype != x.dtype or adaln_shift.device != x.device):
...
def symmetric_scheduled_stream_gate_residual_linear(
    ...
    """Move FastWAM's x+gate*projection into the W4A4 GEMM epilogue."""
    x = _common(x, packed, scales, bias, inverse)
    span = _equal_stream_span(x, gains, token_counts)
